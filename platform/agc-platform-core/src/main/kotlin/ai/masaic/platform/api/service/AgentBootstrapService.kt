...
class AgentBootstrapService(
    ...
    suspend fun bootstrapAgentsOnStartup() {
        try {
            ...
            val mockFunctionsDef = agentBootStrapKit.loadClasspathResource(mockFunFileName) ?: "[]"
            val mockFunctions = agentBootStrapKit.loadMockFunctions(mockFunctionsDef)
            if (agents.isEmpty()) {
                log.warn("No agent definitions found in classpath resource: $agentDefFileName")
                return
            }

            agentBootStrapKit.bootstrapAgents(agents, mockFunctions)
...
@Component
class AgentBootStrapKit(private val agentService: AgentService,
     ...
     */
    suspend fun loadAgentDefinitions(agentDefJson: String): List<PlatformAgent> {
        val agents: List<PlatformAgent> = mapper.readValue(agentDefJson)
        log.info("Loaded ${agents.size} agent definitions")
     ...
     * @return List of PlatformAgent definitions
     */
    suspend fun loadMockFunctions(mockFunDefJson: String): Map<String, BootstrapMockFunction> {
        val functions: List<BootstrapMockFunctionDef> = mapper.readValue(mockFunDefJson)
        log.info("Loaded ${functions.size} functions")
    ...
    fun loadClasspathResource(fileName: String): String? {
        val resource = ClassPathResource(fileName)
        return  if (!resource.exists()) {
            log.warn("Definitions file not found: $fileName")
            null
        }else {
            val fileContent = resource.inputStream.bufferedReader().use { it.readText() }
            log.info { "loaded content from file: $fileName" }
            fileContent
        }
    }
                ...
                try {
                    ...
                    agentTools.forEach { tool ->
                        when (tool) {
                            ...
                            is FileSearchTool -> possibleToBootstrap = true
                                ...
                                if (tool.serverUrl.endsWith("mock.masaic.ai/api/mcp")) {
                                    val mocks: List<MockFunctionDefinition> =
                                        tool.allowedTools.mapNotNull { name ->
                                            ...
                                            val def = mockFunctionRepository.upsert(
                                                boot.function.copy(
                                    ...
                                    val mockServer = platformMcpService.createMockServer(
                                        CreateMockMcpServerRequest(
                                            id = tool.serverLabel,
                                            serverLabel = tool.serverLabel,
                                            toolIds = mocks.map { it.id })
                                    )
                                    agentTools.remove(tool)
                                    val mcpServer = MCPTool(
                                        type = "mcp",
                                        serverLabel = mockServer.serverLabel,
                                    ...
                                    agentTools += MCPTool(
                                        ...
                                        serverLabel = mockServer.serverLabel,
                                        serverUrl = mockServer.url,
                                        allowedTools = mocks.map { it.functionBody.name })
                                }
                            }

                            is PyFunTool -> {
                                val pythonFunction = FunctionCreate(
                                    name = tool.functionDetails.name,
                                    description = tool.functionDetails.description,
                                    code = tool.code,
                                    inputSchema = tool.functionDetails.parameters
                                )
